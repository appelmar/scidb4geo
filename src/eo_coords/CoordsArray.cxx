...
namespace scidb4geo {
    ...
    CoordsChunkIterator::CoordsChunkIterator(CoordsArrayIterator const& arrayIterator, DelegateChunk const* chunk, int iterationMode) : DelegateChunkIterator(chunk, iterationMode & ~(INTENDED_TILE_MODE | IGNORE_NULL_VALUES | IGNORE_DEFAULT_VALUES)),
    ...
    CoordsArrayIterator::CoordsArrayIterator(CoordsArray const& array, AttributeID outAttrID, AttributeID inAttrID) : DelegateArrayIterator(array, outAttrID, array.getInputArray()->getConstIterator(inAttrID)),
    ...
    CoordsArray::CoordsArray(std::shared_ptr<Query>& query, std::shared_ptr<Array> const& array_in, ArrayDesc const& desc_out, bool tile) : DelegateArray(desc_out, array_in),
                                                                                                                                            _xidx(-1),
                                                                                                                                            _yidx(-1),
                                                                                                                                            _tidx(-1),
                                                                                                                                            _runInTileMode(tile) {
        ...
        if (_srs.arrayname.empty()) {
            ...
            for (uint32_t i = 0; i < array_in->getArrayDesc().getDimensions().size(); ++i) {
                ...
                if (d.getBaseName().compare(_srs.xdim) == 0)
                    _xidx = i;
                else if (d.getBaseName().compare(_srs.ydim) == 0)
                    _yidx = i;
            }
        }
