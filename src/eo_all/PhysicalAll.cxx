...
namespace scidb4geo {
    ...
    class PhysicalAll : public PhysicalOperator {
       public:
        PhysicalAll(string &logicalName, const string &physicalName, const Parameters &parameters, const ArrayDesc &schema) : PhysicalOperator(logicalName, physicalName, parameters, schema) {
        }
        ...
        void preSingleExecute(std::shared_ptr<Query> query) {
            ...
            for (size_t i = 0; i < arrays.size(); ++i) {
                ...
                for (size_t iD = 0; iD < arrays[i].getDimensions().size(); ++iD) {
                    ...
                    if (isSpatial) {
                        if (d.getBaseName().compare(srs.xdim) == 0)
                            xdim_idx = iD;
                        else if (d.getBaseName().compare(srs.ydim) == 0)
                            ydim_idx = iD;
                    }
                    if (isTemporal) {
