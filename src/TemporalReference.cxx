...
namespace scidb4geo {
    ...
    TPoint::TPoint(string str) {
        ...
        if (_month >= 0) {
            ...
            if (_dow < 0)
                d = date(_year, 1, 1) + date_duration((_week - 1) * 7);
            else
                d = date(_year, 1, 1) + date_duration((_week - 1) * 7 + (_dow - 1));
            _resolution = DAY;
    ...
    TReference::TReference() : _t0(NULL), _dt(NULL) {
        _r = NONE;
    }
    ...
    TPoint TReference::datetimeAtIndex(int index) {
        if (_t0 == NULL || _dt == NULL) return TPoint();
        return *_t0 + index * *_dt;
    }

    int64_t TReference::indexAtDatetime(TPoint &t) {
        if (_t0 == NULL || _dt == NULL) return -pow(2, 63);
        ...
        double ym;
        if ((_dt->_yd * 12 + _dt->_md) == 0)
            ym = NAN;
        else
            ym = (dif._yd * 12 + dif._md) / (_dt->_yd * 12 + _dt->_md);
        double d;
        if (_dt->_dd.days() == 0)
            d = NAN;
        else
            d = dif._dd.days() / _dt->_dd.days();
        double tt;
        if (_dt->_td.total_seconds() == 0)
            tt = NAN;
        else
            tt = dif._td.total_seconds() / _dt->_td.total_seconds();  // TODO: Fractional seconds?
