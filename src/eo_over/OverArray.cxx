...
namespace scidb4geo {
    ...
    OverArray::OverArray(std::shared_ptr<Query> &query, ArrayDesc const &descA, ArrayDesc const &descB, ArrayDesc const &descC)
        : _desc_A(descA), _desc_B(descB), _desc_C(descC), _xidx_A(-1), _xidx_B(-1), _yidx_A(-1), _yidx_B(-1), _tidx_A(-1), _tidx_B(-1), _ninstances(0), _instance_id(INVALID_INSTANCE) {
        assert(query);
        _query = query;
        ...
        if (srsAB.size() != 2) {
            ...
            for (uint32_t i = 0; i < _desc_A.getDimensions().size(); ++i) {
                ...
                if (d.getBaseName().compare(_srs_A.xdim) == 0)
                    _xidx_A = i;
                else if (d.getBaseName().compare(_srs_A.ydim) == 0)
                    _yidx_A = i;
            }
            for (uint32_t i = 0; i < _desc_B.getDimensions().size(); ++i) {
                d = _desc_B.getDimensions()[i];
                if (d.getBaseName().compare(_srs_B.xdim) == 0)
                    _xidx_B = i;
                else if (d.getBaseName().compare(_srs_B.ydim) == 0)
                    _yidx_B = i;
            }
